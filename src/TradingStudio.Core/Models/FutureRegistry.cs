    ...
    private static FutureRegistry Parse(JsonElement root)
    {
        ...
        foreach (var s in root.GetProperty("symbols").EnumerateArray())
        {
            var p = new Future
            {
                ...
                DeliveryMarginAdjust = TryGetDouble(s, "deliveryMarginAdjust"),
                DeliveryMarginMonths = (int)TryGetDouble(s, "deliveryMarginMonths"),
                HolidayMarginAdjust = TryGetDouble(s, "holidayMarginAdjust"),
