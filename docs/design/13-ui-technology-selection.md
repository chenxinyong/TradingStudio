...
绩效面板:  OxyPlot (权益曲线/回撤图)       ← Phase 2-3
          LiveCharts2                      ← ⏸️ Phase 4（推迟）
数据表格:  WPF 原生 DataGrid              ← Phase 2-3
          HandyControl DataGrid           ← ⏸️ Phase 4（推迟）
...
│                                                            │
│  框架:    WPF (.NET 10, x64)                               │
│  UI 库:   WPF 原生控件（Phase 2-3）                         │
│           HandyControl 3.5 ← Phase 4 按需引入               │
│  K线图:   OxyPlot (MIT) ← Phase 2 主力                     │
│  备选:    FancyCandles (GPL-3.0) ← OxyPlot 不满足时切换     │
│  绩效图:  OxyPlot (MIT) ← 权益曲线/回撤图，Phase 2-3        │
│          LiveCharts2 (MIT) ← Phase 4 按需替换               │
...
> - ⏸️ SignalR Client — 待 Phase 3 实施
> - ⏸️ HandyControl / LiveCharts2 — 推迟到 Phase 4（第一性原理验证结论：OxyPlot + WPF 原生控件够用）
> - ⏸️ FancyCandles 备选方案 — 暂未验证，OxyPlot 目前满足需求
>
> **2026-06-14 第一性原理验证更新：** HandyControl 和 LiveCharts2 从 Phase 3 推迟到 Phase 4。原则 FP4（复杂度是可靠性的敌人）— OxyPlot 一个库能画所有图表，WPF 原生控件够用。详见 [监控客户端设计验证](plans/moonlit-crafting-crab.md)。
>
> **相关文档：**
