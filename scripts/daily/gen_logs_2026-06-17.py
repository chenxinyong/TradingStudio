"""
Generate 2026-06-17 trading logs.
- Stocks: Sina API (hq.sinajs.cn)
- Futures: AKShare (futures_zh_daily_sina)
"""
...
index_codes = ["sh000001","sz399001","sz399006","sh000688","sz399317","sh000016","sh000300","sh000905","sz399852"]
index_names = {"sh000001":"上证指数","sz399001":"深证成指","sz399006":"创业板指","sh000688":"科创50",
    "sz399317":"国证A股","sh000016":"上证50","sh000300":"沪深300","sh000905":"中证500","sz399852":"中证1000"}

# Futures definitions
futures_list = [
    ("AU0","黄金 AU","贵金属"),("AG0","白银 AG","贵金属"),
    ("CU0","铜 CU","有色金属"),("AL0","铝 AL","有色金属"),
    ("ZN0","锌 ZN","有色金属"),("NI0","镍 NI","有色金属"),
    ("SN0","锡 SN","有色金属"),("PB0","铅 PB","有色金属"),
    ("SC0","原油 SC","能源"),("FU0","燃料油 FU","能源"),
    ("TA0","PTA TA","化工"),("MA0","甲醇 MA","化工"),
    ("V0","PVC V","化工"),("L0","LLDPE L","化工"),("EG0","乙二醇 EG","化工"),
    ("RB0","螺纹 RB","黑色系"),("HC0","热卷 HC","黑色系"),("I0","铁矿 I","黑色系"),
    ("JM0","焦煤 JM","黑色系"),("J0","焦炭 J","黑色系"),
    ("M0","豆粕 M","农产品"),("Y0","豆油 Y","农产品"),("P0","棕榈油 P","农产品"),
    ("SR0","白糖 SR","农产品"),("CF0","棉花 CF","农产品"),("RM0","菜粕 RM","农产品"),
    ("LC0","碳酸锂 LC","新能源"),("SI0","工业硅 SI","新能源"),
]

if_futures_list = [("IF0","IF","沪深300"),("IC0","IC","中证500"),("IM0","IM","中证1000"),("IH0","IH","上证50")]

# ============ STEP 1: Fetch stock data from Sina ============
print("[1/4] Fetching stock data from Sina API...")

all_codes = index_codes + [s[0] for s in all_stocks]
stock_data = {}
...
for i in range(0, len(all_codes), 80):
    ...
    try:
        ...
        for line in text.strip().split('\n'):
            ...
            if m:
                stock_data[m.group(1)] = m.group(2).split(',')
    except Exception as e:
        print(f"  Batch {i} error: {e}")
    time.sleep(0.25)

print(f"  Fetched {len(stock_data)} stocks/indices")

# Parse stock prices
stock_prices = {}
for code, name, layer in all_stocks:
...
for ic in index_codes:
    if ic in index_prices:
        ...
        print(f"    {index_names.get(ic, n)}: {p:.2f} ({c:+.2f}%)")

# Stock statistics
all_with_chg = [(code, name, stock_prices[code][1], stock_prices[code][0], layer)
    for code, name, layer in all_stocks if code in stock_prices]
...
# ============ STEP 2: Fetch futures data from AKShare ============
print("\n[2/4] Fetching futures data from AKShare...")

futures_data = {}
for sym, name, cat in futures_list + if_futures_list:
    try:
        df = ak.futures_zh_daily_sina(symbol=sym)
        df['date'] = pd.to_datetime(df['date']).dt.date
        t = df[df['date'] == TODAY_DATE]
        if len(t) == 0:
            print(f"  {sym} {name}: no data for today")
            continue
        t = t.iloc[0]
        today_idx = df[df['date'] == TODAY_DATE].index[0]
        y = df.iloc[today_idx - 1] if today_idx > 0 else None
        y2 = df.iloc[today_idx - 2] if today_idx > 1 else None

        close = float(t['close'])
        prev_close = float(y['close']) if y is not None else None
        prev_hold = float(y['hold']) if y is not None else None
        chg = round((close - prev_close) / prev_close * 100, 2) if prev_close and prev_close != 0 else None
        hold_chg = int(float(t['hold']) - prev_hold) if prev_hold is not None else None
        vol = int(t['volume'])
        hold = int(t['hold'])

        # 3-day trend
        if y2 is not None:
            c_3d_ago = float(y2['close'])
            if close > c_3d_ago * 1.03: trend = "偏强"
            elif close < c_3d_ago * 0.97: trend = "偏弱"
            else: trend = "震荡"
        else:
            trend = "—"

        futures_data[sym] = {
            "name": name, "cat": cat, "close": close, "chg_pct": chg,
            "vol": vol, "hold": hold, "hold_chg": hold_chg, "trend": trend
        }
        print(f"  {sym} {name}: close={close} chg={chg:+.2f}% hold_chg={hold_chg}")
    except Exception as e:
        print(f"  {sym} {name}: ERROR - {e}")
    time.sleep(0.15)

# ============ STEP 3: Generate Stock Log ============
print("\n[3/4] Generating stock log...")
...
if abnormal:
    for code, name, chg, price, layer in abnormal[:8]:
        ...
        sl("| **{}** | {} | {} | **{:+.2f}%** | 市场驱动 | 重点关注 |".format(name, clean_code, direction, chg))
else:
    sl("| 今日无涨跌幅>5%的异动个股 | | | | | | |")
sl("")
sl("**说明**：重点关注 / 观察 / 忽略")
...
sl("")
sl("- 研究框架：[[AI产业链深度分析]]")
sl("- 期货日志：[[01-Daily/期货/{}]]".format(TODAY))
sl("- 本周复盘：待创建")
...
# ============ STEP 4: Generate Futures Log ============
print("\n[4/4] Generating futures log...")
...
fl("# 期货交易日志 - " + TODAY)
fl("")
fl("> **夜盘归属说明**：本日志记录 {} 白天日盘 + {} 夜盘".format(TODAY, PREV_TRADING_DAY))
fl("")
fl("> **数据日期说明**：商品期货日K线数据为{}（今日日盘）收盘数据。股指期货数据来自连续合约。".format(TODAY))
fl("")
fl("---")
fl("")
...
# Calculate sector averages and overall index
sector_changes = {}
all_f_changes = []
for sym, name, cat in futures_list:
    if sym in futures_data and futures_data[sym].get("chg_pct") is not None:
        ch = futures_data[sym]["chg_pct"]
        sector_changes.setdefault(cat, []).append(ch)
        all_f_changes.append(ch)

sector_avgs = {cat: sum(vals)/len(vals) for cat, vals in sector_changes.items() if vals}
total_avg = sum(all_f_changes) / len(all_f_changes) if all_f_changes else 0

fl("### 商品指数（基于28个跟踪合约估算）")
fl("| 指数 | 估算涨跌幅 | 趋势判断 | 备注 |")
fl("|------|-----------|---------|------|")
fl("| **综合商品指数** | **{:+.2f}%** | {} | 板块加权平均 |".format(total_avg, "震荡偏弱" if total_avg < -0.3 else ("震荡偏强" if total_avg > 0.3 else "震荡")))
...
fl("")
fl("**文华商品方向（估算）**：{}（综合指数 {:+.2f}%）".format(
    "震荡偏弱" if total_avg < -0.3 else ("震荡偏强" if total_avg > 0.3 else "震荡"), total_avg))
fl("")
...
# Spot index prices for basis calculation
spot_map = {"IF":"沪深300","IC":"中证500","IM":"中证1000","IH":"上证50"}
spot_code_map = {"IF":"sh000300","IC":"sh000905","IM":"sz399852","IH":"sh000016"}

for sym, prefix, name in if_futures_list:
    if sym in futures_data:
        d = futures_data[sym]
        spot_price = None
        idx_code = spot_code_map.get(prefix)
        if idx_code and idx_code in index_prices:
            spot_price = index_prices[idx_code][0]
        if spot_price:
            basis = d['close'] - spot_price
            basis_rate = basis / spot_price * 100
            fl("| **{}**（{}） | **{:.2f}** | **{:+.2f}%** | {:.2f} | **{:+.2f}** | **{:+.2f}%** | {:+,d} |".format(
                sym, name, d['close'], d['chg_pct'], spot_price, basis, basis_rate, d['hold_chg']))
        else:
            fl("| **{}**（{}） | **{:.2f}** | **{:+.2f}%** | — | — | — | {:+,d} |".format(
                sym, name, d['close'], d['chg_pct'], d['hold_chg']))
    else:
...
for sym, prefix, name in if_futures_list:
    if sym in futures_data:
        ...
        if idx_code and idx_code in index_prices:
            spot_price = index_prices[idx_code][0]
            basis = d['close'] - spot_price
            btype = "贴水" if basis < 0 else "升水"
            oi_dir = "增加" if d['hold_chg'] > 0 else "减少"
            basis_notes.append("{}（{}）{}{:.2f}点（{:.2f}%），持仓{}".format(
                name, prefix, btype, abs(basis), abs(basis)/spot_price*100, oi_dir))
if basis_notes:
    for bn in basis_notes:
        fl("- " + bn)
fl("")
...
futures_by_cat = {}
for sym, name, cat in futures_list:
    futures_by_cat.setdefault(cat, []).append((sym, name))
...
for cat in ["贵金属","有色金属","能源","化工","黑色系","农产品","新能源"]:
    ...
    fl(table_header)
    for sym, name in futures_by_cat.get(cat, []):
        if sym not in futures_data:
            fl("| {} | 数据获取失败 | — | — | — | — | 无 |\n".format(name))
            continue
        d = futures_data[sym]
        chg = d['chg_pct'] if d['chg_pct'] is not None else 0
        hc = d['hold_chg']
        hc_str = "{:+,}".format(hc) if hc is not None else "—"
        trend = d.get('trend', '—')
        fl("| {} | {} | **{:+.2f}%** | {} | {} | — | 无 |\n".format(
            name, d['close'], chg, hc_str, trend))
        if abs(chg) >= 2:
            abnormal_futs.append((name, cat, chg, hc, hc_str, sym))
    fl("")

# Abnormal futures
abnormal_futs.sort(key=lambda x: abs(x[2]), reverse=True)
...
fl("|------|------|--------|-----------|---------|---------|")
if abnormal_futs:
    for name, cat, chg, oi_chg, oi_str, sym in abnormal_futs:
        direction = "↑" if chg > 0 else "↓"
        oi_type = "增仓" if oi_chg and oi_chg > 0 else ("减仓" if oi_chg and oi_chg < 0 else "—")
...
if sector_avgs:
    ...
    mkt_style = "震荡偏弱" if total_avg < -0.3 else ("震荡偏强" if total_avg > 0.3 else "震荡")
    fl("- 文华商品方向（规则一）：{}（估算综合指数 {:+.2f}%）".format(mkt_style, total_avg))
    fl("- 当前市场风格（趋势/震荡/无序）：**{}**（板块分化）".format(mkt_style))
    fl("- 盈利效应（好/一般/差）：**一般**")
...
fl("## 八、链接")
fl("")
fl("- 股票日志：[[01-Daily/股票/{}]]".format(TODAY))
...
print("\n" + "=" * 60)
print("✅ ALL DONE!")
print("  股票日志: 01-Daily/股票/{}.md".format(TODAY))
print("  期货日志: 01-Daily/期货/{}.md".format(TODAY))
print("  自选股: {}/{} 获取成功, {}涨{}跌{}平".format(len(stock_prices), len(all_stocks), up_c, down_c, flat_c))
print("  期货: {}/{} 获取成功".format(len(futures_data), len(futures_list) + len(if_futures_list)))
print("=" * 60)
