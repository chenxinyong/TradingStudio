...
# ---- 拉 002531 天顺风能 日线（不复权） ----
df = ak.stock_zh_a_daily(symbol='sz002531', adjust='')
df = df.rename(columns=str.lower).sort_values('date').reset_index(drop=True)
df['date'] = pd.to_datetime(df['date'])

print(f"=== 002531 日线：{len(df)} 根，{df['date'].iloc[0].date()} ~ {df['date'].iloc[-1].date()} ===")
df.to_csv('tmp_002531.csv', index=False)
